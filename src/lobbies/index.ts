...
export const withLobbyCommands =
  () => (reactor: Reactor<Bun.Socket<SessionData>>) => {
    ...
    .on("lobby/get", (cmd, exchange) => {
      ...
      const params = [id, lobby.isLocked ? 'locked' : '', !lobby.isVisible ? 'hidden' : ''].filter(it => !!it)
      ...
      for (const [key, value] of lobby.data.entries())
        ...
        // TODO(trimsock): Serialize kv-pairs
          exchange.stream({ kvParams: [[key, value]], chunks: [{ text: key, isQuoted: true}, {text: "=", isQuoted: false}, {text: value, isQuoted: true}] })
